...
def get_help(module_file_name, output=False):
    ...
    if isinstance(module_file_name, ModuleType):
        module_file_name = module_file_name.__file__
    elif not isinstance(module_file_name, str):
        raise FileTypeError("Wrong file type. This function can only handle python files ('.py') or python modules.")
    elif os.path.isdir(module_file_name):
        raise FileTypeError("Wrong file type. This function cannot handle directories.")
    else:
        extension = os.path.splitext(os.path.basename(module_file_name))[-1]
        if extension != '.py':
            raise FileTypeError("Wrong file type. This function can only handle python files ('.py') or python modules.")
...
def save_as_text(module_file_name, outputfile=None, outputdir=''):
    ...
    astr = get_help(module_file_name, output=True)
    if not outputfile:
        outputfile = '.'.join(module_file_name.split('.')[:-1])+'.txt'
...
def save_as_html(module_file_name, outputfile=None, outputdir=''):
    ...
    astr += '\n <title>Software Documentation: {0}</title>'.format(modulename+'.py')
    astr += '\n <link href="docu.css" rel="stylesheet" type="text/css">'
    astr += '\n</head>'
    astr += '\n<body>'
    astr += '\n <div class="back-title">'
    astr += ('\n  <h1 class="title">Software Documentation: '
             '{0}</h1>').format(modulename+'.py')
    astr += '\n </div>'
    astr += '\n <div class="back-main">'
    astr += '\n <div class="main">'
    ...
    astr += '\n</html>'
    if not outputfile:
        outputfile = os.path.splitext(os.path.basename(module_file_name))[0]+'.html'
    outputfile = os.path.join(outputdir, outputfile)
...
def parse(module_file_name):
    ...
    modulename, extension = os.path.splitext(os.path.basename(module_file_name))
    if extension != '.py':
        raise FileTypeError("Wrong file type. This function can only handle python files ('.py') or python modules.")
    modules = []
    variables = []
